...
const url = "https://gametime.co/nfl-football/packers-at-cardinals-tickets/3-8-2026-glendale-az-state-farm-stadium/events/677c937b707c8136ad2133e8?queryId=2ad66b0ab8729716e09c684705f4fb08&resultPosition=3&searchIndex=performers";
const outputFile = "fb_gt_tbd8.txt";
const collectedTickets = new Set();
const source = "Gametime";
const baseUrl = "https://gametime.co";
const date = "tbd8";
const homeTeam = "Cardinals";
const awayTeam = "Packers";
...
const initializeDb = async () => {
    try {
        await runAsync(`
            CREATE TABLE IF NOT EXISTS ticketsfb (
        ...
        const deleteResult = await runAsync(
            `DELETE FROM ticketsfb WHERE source = ? AND date = ? AND home_team = ?`,
            ...
            try {
                ...
                if (section && row && price) {
                    ...
                    if (!collectedTickets.has(ticketInfo)) {
                        ...
                        try {
                            await runAsync(
                                `INSERT OR REPLACE INTO ticketsfb (date, home_team, away_team, section, row, price, estimated_price, url, source)
                                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
